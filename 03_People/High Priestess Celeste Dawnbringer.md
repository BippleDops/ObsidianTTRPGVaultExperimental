...
## Portrait
![[04_Resources/Assets/Portraits/portrait_high_priestess_celeste_dawnbringer_dramatic.png]]
...
- Loses `dice: `dice: 1d4`` months of natural lifespan per major healing session
...
- Requires ongoing maintece sessions
...
**Desperate Healing (1/Day).** When an ally drops to 0 hit points, Celeste can immediately teleport to them and cast *heal* as a reaction, but takes `dice: `dice: 2d6`` necrotic damage.
...
**Corruption Cleansing (3/Day).** Touch attack that removes crystal corruption effects. Target makes **DC 18** Constitution save; success removes one corruption level, failure deals `dice: `dice: 4d8`` radiant damage.
